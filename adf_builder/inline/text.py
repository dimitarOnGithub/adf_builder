...
from adf_builder.marks import MarkMixin, MarkEnum, Link, Color
...
class Text(InlineNode):
    ...
    def to_dict(self):
        ...
        if self._marks:
            _marks_payload = [mark.to_dict() for mark in self._marks]
        ...
        if mark_type == MarkEnum.CODE:
            for existing_mark in self._marks:
                if not isinstance(existing_mark, MarkEnum):
                    raise ValueError("Code blocks can only be combined with a link")
